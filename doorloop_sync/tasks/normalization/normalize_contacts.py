...
def run():
    logger.info("Starting normalization for contacts...")
    try:
        supabase_client = get_supabase_client()
        raw_table_name = "doorloop_raw_contacts"
        normalized_table_name = "doorloop_normalized_contacts"
        response = supabase_client.supabase.table(raw_table_name).select("data").execute()
        raw_records = response.data

        if not raw_records:
            logger.info("No raw contacts data to normalize. Task complete.")
            return
        ...
        if normalized_records:
            ...
            logger.info(f"Successfully normalized and upserted {len(normalized_records)} contacts records.")
    except Exception as e:
        logger.error(f"An error occurred during contacts normalization: {e}", exc_info=True)
        raise
