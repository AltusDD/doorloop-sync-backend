...
def run():
    ...
    try:
        ...
        raw_table_name = "doorloop_raw_units"
        normalized_table_name = "doorloop_normalized_units"
        response = supabase_client.supabase.table(raw_table_name).select("data").execute()
        raw_records = response.data

        if not raw_records:
            logger.info("No raw units data to normalize. Task complete.")
            return
        ...
        if normalized_records:
            ...
            logger.info(f"Successfully normalized and upserted {len(normalized_records)} units records.")
    except Exception as e:
        logger.error(f"An error occurred during units normalization: {e}", exc_info=True)
        raise
