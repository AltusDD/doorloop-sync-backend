...
def run():
    logger.info("Starting normalization for defaults...")
    try:
        supabase_client = get_supabase_client()
        raw_table_name = "doorloop_raw_defaults"
        normalized_table_name = "doorloop_normalized_defaults"
        response = supabase_client.supabase.table(raw_table_name).select("data").execute()
        raw_records = response.data

        if not raw_records:
            logger.info("No raw defaults data to normalize. Task complete.")
            return
        ...
        if normalized_records:
            ...
            logger.info(f"Successfully normalized and upserted {len(normalized_records)} defaults records.")
    except Exception as e:
        logger.error(f"An error occurred during defaults normalization: {e}", exc_info=True)
        raise
