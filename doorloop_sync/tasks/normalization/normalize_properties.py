...
def run():
    ...
    try:
        ...
        if not raw_records:
            logger.info("No raw properties data to normalize. Task complete.")
            return
        ...
        if normalized_records:
            ...
            logger.info(f"Successfully normalized and upserted {len(normalized_records)} properties records.")
    except Exception as e:
        logger.error(f"An error occurred during properties normalization: {e}", exc_info=True)
        raise
