...
def sync_properties():
    ...
    table_name = 'doorloop_raw_properties'
    endpoint = 'properties'
    ...
    try:
        ...
        if not all_records:
            logger.info("No records found for properties.")
            return

        logger.info(f"Fetched {len(all_records)} total records for properties.")
        ...
        logger.error(f"An error occurred during raw sync for properties: {e}", exc_info=True)
        raise
