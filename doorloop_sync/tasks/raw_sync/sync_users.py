...
def sync_users():
    ...
    try:
        ...
        if not all_records:
            logger.info("No records found for users.")
            return

        logger.info(f"Fetched {len(all_records)} total records for users.")
        ...
        logger.error(f"An error occurred during raw sync for users: {e}", exc_info=True)
        raise
