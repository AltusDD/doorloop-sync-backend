...
def sync_accounts():
    ...
    try:
        ...
        if not all_records:
            logger.info("No records found for accounts.")
            return

        logger.info(f"Fetched {len(all_records)} total records for accounts.")
        ...
        logger.error(f"An error occurred during raw sync for accounts: {e}", exc_info=True)
        raise
