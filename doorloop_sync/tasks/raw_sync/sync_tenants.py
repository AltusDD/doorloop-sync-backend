...
def sync_tenants():
    ...
    table_name = 'doorloop_raw_tenants'
    endpoint = 'tenants'
    ...
    try:
        ...
        if not all_records:
            logger.info("No records found for tenants.")
            return

        logger.info(f"Fetched {len(all_records)} total records for tenants.")
        ...
        logger.error(f"An error occurred during raw sync for tenants: {e}", exc_info=True)
        raise
