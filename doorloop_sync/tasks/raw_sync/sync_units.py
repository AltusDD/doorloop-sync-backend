...
def sync_units():
    ...
    try:
        ...
        if not all_records:
            logger.info("No records found for units.")
            return

        logger.info(f"Fetched {len(all_records)} total records for units.")
        ...
        logger.error(f"An error occurred during raw sync for units: {e}", exc_info=True)
        raise
