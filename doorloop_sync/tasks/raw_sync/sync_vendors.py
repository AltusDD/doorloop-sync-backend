...
def sync_vendors():
    ...
    table_name = 'doorloop_raw_vendors'
    endpoint = 'vendors'
    ...
    try:
        ...
        if not all_records:
            logger.info("No records found for vendors.")
            return

        logger.info(f"Fetched {len(all_records)} total records for vendors.")
        ...
        logger.error(f"An error occurred during raw sync for vendors: {e}", exc_info=True)
        raise
