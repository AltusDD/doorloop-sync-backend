...
def run():
    logging.info("🔄 Starting normalization for insurance_policies")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_insurance_policies"
    normalized_table = "doorloop_normalized_insurance_policies"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for insurance_policies. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} insurance_policies records")
