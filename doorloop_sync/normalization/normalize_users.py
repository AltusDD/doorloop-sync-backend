...
def run():
    logging.info("🔄 Starting normalization for users")
    ...
    if not raw_records:
        logging.info("📭 No raw data found for users. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} users records")
