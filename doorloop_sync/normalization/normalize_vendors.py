...
def run():
    logging.info("🔄 Starting normalization for vendors")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_vendors"
    normalized_table = "doorloop_normalized_vendors"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for vendors. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} vendors records")
