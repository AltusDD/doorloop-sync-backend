...
def run():
    logging.info("🔄 Starting normalization for tasks")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_tasks"
    normalized_table = "doorloop_normalized_tasks"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for tasks. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} tasks records")
