...
def run():
    logging.info("🔄 Starting normalization for inspections")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_inspections"
    normalized_table = "doorloop_normalized_inspections"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for inspections. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} inspections records")
