...
def run():
    logging.info("🔄 Starting normalization for portfolios")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_portfolios"
    normalized_table = "doorloop_normalized_portfolios"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for portfolios. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} portfolios records")
