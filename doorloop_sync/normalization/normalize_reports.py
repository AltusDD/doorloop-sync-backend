...
def run():
    logging.info("🔄 Starting normalization for reports")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_reports"
    normalized_table = "doorloop_normalized_reports"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for reports. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} reports records")
