...
def run():
    ...
    raw_table = "doorloop_raw_communications"
    normalized_table = "doorloop_normalized_communications"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for communications. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} communications records")
