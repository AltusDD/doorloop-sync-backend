...
def run():
    logging.info("🔄 Starting normalization for recurring_credits")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_recurring_credits"
    normalized_table = "doorloop_normalized_recurring_credits"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for recurring_credits. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} recurring_credits records")
