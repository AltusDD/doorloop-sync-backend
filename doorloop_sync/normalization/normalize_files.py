...
def run():
    logging.info("🔄 Starting normalization for files")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_files"
    normalized_table = "doorloop_normalized_files"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for files. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} files records")
