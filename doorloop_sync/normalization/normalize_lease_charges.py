...
def run():
    logging.info("🔄 Starting normalization for lease_charges")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_lease_charges"
    normalized_table = "doorloop_normalized_lease_charges"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for lease_charges. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} lease_charges records")
