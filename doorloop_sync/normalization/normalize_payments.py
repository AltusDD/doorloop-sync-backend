...
def run():
    logging.info("🔄 Starting normalization for payments")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_payments"
    normalized_table = "doorloop_normalized_payments"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for payments. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} payments records")
