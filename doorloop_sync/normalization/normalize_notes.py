...
def run():
    logging.info("🔄 Starting normalization for notes")

    supabase = SupabaseClient()
    raw_table = "doorloop_raw_notes"
    normalized_table = "doorloop_normalized_notes"

    raw_records = supabase.get_raw_records(raw_table)
    if not raw_records:
        logging.info("📭 No raw data found for notes. Task complete.")
        return
    ...
    logging.info(f"✅ Normalized {len(normalized)} notes records")
