...
def upsert_raw_doorloop_data(
    ...
    # === 1. Prepare and send to raw_doorloop_data ===
    ...
    for i, record in enumerate(records):
        try: # CRITICAL FIX: Try-except for each record processing
        ...
        except Exception as e: # Catch errors during single record processing
            ...
            # Do NOT re-raise, continue to next record. This record will be skipped.
            continue

    if prepared_raw_doorloop_data:
        print(f"DEBUG_SUPABASE_CLIENT: Upserting {len(prepared_raw_doorloop_data)} records to raw_doorloop_data URL: {raw_doorloop_data_table_url}")
        try:
