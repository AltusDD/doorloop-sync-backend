...
def sync():
    ...
    while True:
        ...
        if response.status_code != 200:
            print(f"[ERROR] DoorLoop /leases failed at page {page}: {response.status_code}")
        ...
        print(f"[INFO] Page {page}: {len(batch)} records from /leases")
    ...
    print(f"[SUCCESS] Synced {total_synced} from /leases")
    upsert = upsert_records("leases", all_data)
