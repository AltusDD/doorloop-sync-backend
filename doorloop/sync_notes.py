...
def sync():
    ...
    while True:
        ...
        if response.status_code != 200:
            print(f"[ERROR] DoorLoop /notes failed at page {page}: {response.status_code}")
        ...
        print(f"[INFO] Page {page}: {len(batch)} records from /notes")
    ...
    print(f"[SUCCESS] Synced {total_synced} from /notes")
    upsert = upsert_records("notes", all_data)
